...
//DTO로 변환하여 반환하는 이유
//1. 비지니스 로직을 캡슐화 하기 위함이다.
// - 대부분의 회사들은 db의 스키마를 아는것을 원치 않는다.
// - dto처럼 다른 객체로 바꿔서 전달하면 외부 사용자에게 서비스 내부의 로직, db구조를 숨길 수 있다.
//2. 필요한 정보를 엔티티가 전부 포함하지 않는 경우가 있다.
// 서비스 도중 에러가 발생하면 에러 메시지는 어디에 포함심켜서 보내야 하는가
