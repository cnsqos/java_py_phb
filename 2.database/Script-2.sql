...
#id : 2, 제품명 : 키보드, 카테고리 : 전자제품, 가격 : 45000, 재고 : 30
...
#id : 9, 제품명 : 커피머신, 카테고리 : 전자제품, 가격 : 300000, 재고 : 3
...
VALUES
  ...
  (2, '키보드', '전자제품', 45000, 30),
  (3, '마우스', '전자제품', 170000, 50),
  (4, '의자', '가구', 350000, 15),
  (5, '책상', '가구', 1700000, 7),
  (6, '텀블러', '생활용품', 50000, 80),
  (7, '물병', '생활용품', 3000, 120),
  (8, '청소기', '전자제품', 1900000, 10),
  (9, '커피머신', '전자제품', 300000, 3),
  (10, '캠핑의자', '레저', 270000, 40);


#전자제품 데이터만 조회하기

select * from product where category = '전자제품';
...
select * from product where category = '전자제품' and price >= 300000;

# 카테고리가 생활용품이면서 재고가 50개 이상인 상품을 조회하세요

select * from product where category = '생활용품' and stock >= 50;

# 1985년 한 해 동안 입사한 사원의 사번, 이름 입사일을 조회하세요

select emp_no, first_name, hire_date from employees where hire_date >= '1985-01-01' and hire_date <= '1985-12-31';

# 컬럼 between A and B
# A와 B사이의 값을 조회할 때 사용 (and랑 같음)
select emp_no, first_name, hire_date from employees where hire_date >= '1985-01-01' and hire_date <= '1985-12-31';
select emp_no, first_name, hire_date from employees where hire_date between '1985-01-01' and '1985-12-31';


# 현재 급여가 60,000 이상 80,000 이하인 사원의 사번, 이름, 급여

select emp_no, salary from salaries s where salary between 60000 and 80000;

# 카테고리가 '전자제품' 또는 '가구'에 해당하는 제품 조회하기
select * from product where category = '전자제품' or category = '가구';

select * from product where category in ('전자제품','가구');

# 상품명이 노트북, 청소기, 커피머신인 상품을 조회하세요

select * from product where name in ('노트북','청소기','커피머신');

# 재고가 5, 10 ,15 중 하나와 일치하는 상품을 조회하세요

select * from product where stock in (5,10,15);

# 카테고리가 가구 또는 레저가 아닌 상품을 조회하세요
select * from product where category not in ('가구','레저');

# 이름이 'M'으로 시작하는 사원들의 사원번호와 이름을 조회하세요

select emp_no, first_name from employees e where first_name like 'M%';

# 이름이 'd'로 끝나는 사원의 사번, 이름을 조회

select emp_no, first_name from employees e where first_name like '%d';

# 이름의 첫글자가 M이면서 총 6글자인 사람의 사원번호와 이름을 조회하세요

select emp_no, first_name from employees e where first_name like 'M_____';

# 이름의 세번째 글짜가 'a'인 사원의 사번과 이름을 조회

select emp_no, first_name from employees e where first_name like '__a%';

# 이름이 H로 시작하면서 6글자 이상인 사원의 사번과 이름을 조회하세요

select emp_no, first_name from employees e where first_name like 'H_____%';

# 이름에 s가 포함되지 않은 사원들의 사번, 이름을 조회하세요

select emp_no, first_name from employees e where first_name not like '%s%';
...
insert into tbl_student (id, name, major, birth)
	values (1, '박길동','화학공학과','1980-01-02');

select * from tbl_student ts ;


#update : 테이블 안에 들어있는 값을 수정하는 키워드
#update set
#컬럼명 1 = 값 1,
#컬럼명 2 = 값 2,
#where 조건식
...
update product set price = 50000 where id = 2;
select * from product;


# 카테고리가 '전자제품'인 상품들의 재고를 5 씩 증가시키세요

update product set stock = stock+5 where category = '전자제품';
select * from product;

# 이름이 '의자'로 끝나는 상품의 가격을 30000원으로 설정하세요

update product set price = 30000 where name like '%의자';
select * from product;

# 카테고리가 '가구'인 상품들의 카테고리를 '인테리어'로 변경하세요
update product set category = '인테리어' where category = '가구';
select * from product;

# 가격이 10만원 이하인 상품들의 가격을 10% 인상하세요

update product set price = price*1.1 where price <= 100000;
select * from product;

# ID가 1번 또는 8번인 상품의 재고를 20으로 수정하세요

update product set stock = 20 where id in (1,8);
select * from product;
...
#char_length(문자열)
#문자 개수를 반환
#한글, 이모지를 포함해서 <실제 글자수>를 보고 싶을 때 사용
select char_length('abc'), CHARACTER_LENGTH('가나다');
...
select * from employees e where char_length(e.first_name) >= 5 ;
...
#LPAD (문자열,길이,대체문자열)
#문자열 전체를 길이가 되도록 만들고 빈 공간은 대체 문자열로 채운다.

select lpad ('HELLO',10,'*');

#ex) 회원 번호를 5자리로 통일하고 싶을 때
select lpad (1235,5,0);
...
insert into email values ('abc@naver.com'),('gildong@gmail.com'),('devdoctor@daum.net');

select substring('abc@naver.com',1,3),substring('gildong@gmail.com',1,7),substring('devdoctor@daum.net',1,9);

select substring(email,1,instr(email,'@')-1) from email;

# '@naver.com'가 포함된 데이터만 조회하기

select * from email where instr(email,'@naver.com') > 0;

#concat(문자열,문자열,...)
#여러 문자열을 이어붙이는 함수
#괄호 안에 들어간 문자열들을 순서대로 이어붙인다.
#개수 제한 없이 원하는 만틈 넣을 수 있다.
